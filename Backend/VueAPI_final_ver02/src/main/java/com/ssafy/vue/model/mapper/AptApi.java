		...
		while (!dongAptNum.isEmpty()) {
			...
			for (int i = 0; i < child.getLength(); i++) {
				...
				if("일련번호".equals(node.getNodeName()))
					...
					apt.setJibun(node.getTextContent());
...
//		List<AptInfoDto> aptlist = api.getAptListApi(LAWD_CD, DONG, NUM_OF_ROWS, PAGE_NO, DEAL_YMD);
//		
